processNDF <- function(filename = NULL, ncols=768)
{
    if(is.null(filename))
        stop("Path to NDF file not given.")
...
loadPairFile <- function(filename = NULL, ndf = NULL, ncols=768)
{
    if(is.null(filename))
        stop("Path to PAIR file not given.")
    if(is.null(ndf))
        stop("NDF data frame not given.")
...
loadSampleDirectory <- function(path = NULL, ndf = NULL, what="Cy3", ncols=768)
{
    if(is.null(path))
        stop("Path to PAIR files not given.")
    if(is.null(ndf))
        stop("NDF data frame not given.")
    if (!what %in% c("Cy3","Cy5","Cy3/Cy5","Cy5/Cy3","Cy3andCy5","Cy5andCy3"))
        stop('parameter "what" must be "Cy3", "Cy5", "Cy3/Cy5", "Cy5/Cy3", "Cy3andCy5" or "Cy5andCy3"')	
